...
<head>
  ...
  <title>Profil Akun Saya - SMKN 2 Purwakarta Libraries</title>

  @include('partials.head', [
    'title' => 'Profil Akun Saya - SMKN 2 Purwakarta Libraries',
    'description' => 'Kelola data profil akun Perpustakaan Digital SMKN 2 Purwakarta.',
    'robots' => 'noindex',
  ])
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Plus Jakarta Sans',sans-serif; }
    ...
    .dropdown-content a { display:flex; align-items:center; gap:10px; padding:10px 18px; color:#334155; font-size:13px; font-weight:600; transition:0.2s; }
    ...
    /* PROFILE CONTENT */
    .profile-hero { padding:36px 0 20px; }
    .profile-hero h1 { font-size:28px; font-weight:800; color:#0f172a; }
    .profile-hero p { font-size:14px; color:var(--text-muted); margin-top:4px; }

    .profile-grid { display:grid; grid-template-columns:1fr 1.3fr; gap:32px; padding-bottom:60px; align-items:start; }
    ...
    /* AVATAR UPLOAD */
    .avatar-center-wrap { text-align:center; padding:10px 0 24px; border-bottom:1px solid var(--border); margin-bottom:22px; }
    .avatar-preview-box { width:100px; height:100px; border-radius:50%; background:#d1fae5; color:var(--primary); display:inline-flex; align-items:center; justify-content:center; font-size:32px; font-weight:800; overflow:hidden; border:3px solid var(--border); margin-bottom:12px; box-shadow:0 4px 10px rgba(0,0,0,0.06); }
    ...
    .form-control:disabled, .form-control[readonly] { background-color:#f8fafc; color:#64748b; cursor:not-allowed; border-color:#e2e8f0; }
    ...
    @media (max-width:992px) { .profile-grid { grid-template-columns:1fr; } .footer-grid { grid-template-columns:1fr 1fr; } }
...
<body>
  ...
  <main class="custom-container">
    <div class="profile-hero">
      ...
      <p>Kelola data login, foto profil, dan kata sandi akun Anda di Perpustakaan SMKN 2 Purwakarta.</p>
    </div>
    ...
    <div class="profile-grid">
      ...
      <!-- SISI KIRI: DATA IDENTITAS TERKUNCI (READ-ONLY) -->
      <div>
        <div class="card-panel">
          <h2>Identitas Resmi Sekolah</h2>
          <p class="sub">Data resmi yang terdaftar di database sekolah dan tidak dapat diubah sendiri.</p>
          ...
          <div class="form-row">
            <div class="form-group">
              <label>Nomor Induk (NIS / NIP) <span class="badge-locked"><i class="fa-solid fa-lock"></i> Terkunci</span></label>
          ...
          <div class="form-group">
            <label>Email Sekolah Terdaftar <span class="badge-locked"><i class="fa-solid fa-lock"></i> Terkunci</span></label>
            <input type="email" class="form-control" value="{{ $user->email }}" disabled>
          </div>

          <p style="font-size:11.5px; color:#64748b; line-height:1.5; margin-top:10px;">
            <i class="fa-solid fa-circle-info"></i> Jika terdapat kesalahan pada nama, NIS/NIP, atau email sekolah, silakan hubungi bagian administrator perpustakaan.
          </p>
        </div>

        <!-- STATUS PINJAMAN BUKU AKTIF -->
        <div class="card-panel">
          <h2>Status Peminjaman Aktif</h2>
          <p class="sub">Buku yang sedang Anda pinjam saat ini.</p>

          @if(isset($activeLoans) && $activeLoans->count() > 0)
            @foreach($activeLoans as $loan)
              <div style="display:flex; align-items:center; gap:12px; padding:10px; border:1px solid #e2e8f0; border-radius:8px; margin-bottom:8px; border-left:4px solid #0c4d2d;">
                <i class="fa-solid fa-book" style="color:var(--primary); font-size:18px;"></i>
                <div style="flex:1;">
                  <div style="font-size:13px; font-weight:700;">{{ $loan->book->title ?? 'Judul Buku' }}</div>
                  <div style="font-size:11px; color:#64748b;">Batas kembali: <strong>{{ $loan->due_date }}</strong></div>
                </div>
                <a href="{{ url('/buku/' . $loan->book_id) }}" style="font-size:11.5px; color:var(--primary); font-weight:700;">Lihat</a>
              </div>
            @endforeach
          @else
            <p style="font-size:12.5px; color:#64748b;">Tidak ada buku yang sedang dipinjam.</p>
          @endif
        </div>
      </div>

      <!-- SISI KANAN: FORM EDIT (USERNAME, FOTO PROFIL, DAN PASSWORD) -->
      <div class="card-panel">
        <h3>Edit Data Diri & Keamanan</h3>
        <p class="sub">Anda dapat mengubah foto profil, username akun, serta memperbarui password.</p>
        ...
        <form action="{{ route('user.profile.update') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <small style="font-size:11px; color:#64748b;">Username ini digunakan untuk masuk/login ke perpustakaan.</small>
          </div>
          ...
          <div style="border-top:1px solid var(--border); padding-top:18px; margin-top:18px;">
            <label style="font-size:13px; font-weight:800; color:#0f172a; margin-bottom:10px; display:block;">
              <i class="fa-solid fa-key"></i> Ganti Kata Sandi (Opsional)
            </label>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.addEventListener('click', function(e) {
        if (!e.target.closest('.main-header')) {
          ...
          document.querySelectorAll('.dropdown-content').forEach(drop => drop.classList.remove('open'));
        }
      });
    });
  </script>
